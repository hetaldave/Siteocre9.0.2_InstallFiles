param(
	[string]$EngineSuffix = 'XcStoreFront_902',
    [string]$SitePrefix = "XcStoreFront_902",
	[string]$ScInstallDirectory = "C:\inetpub\wwwroot",
	[string]$SiteHostHeaderName = "XcStoreFront_902.local.com",
	[string]$SqlServer = ".\",
	[string]$SqlDbPrefix = "XcStoreFront_902",
	[string]$CommerceEngineCertificatePath = "..\XcStoreFront_902.local.cer",
	[string]$CommerceSearchProvider = "SOLR",
	[string]$SolrUrl = "https://localhost:8983/solr",
	[string]$SolrRoot = "C:\Projects\XcStoreFront_902\Solr\solr-6.6.2\",
	[string]$SolrService = "solr-6.6.2-XcStoreFront_902"
)
...
$params = @{
		CommerceServicesPostfix = "$($EngineSuffix)"
        Path = Resolve-Path '.\Configuration\Commerce\Master_SingleServer.json'	
		SiteName = "$($SitePrefix).local.com"
		SiteHostHeaderName = $SiteHostHeaderName 
		InstallDir = "$($ScInstallDirectory)\$($SitePrefix).local.com"
		XConnectInstallDir = "$($ScInstallDirectory)\$($SitePrefix).xconnect.local.com"
		CertificateName = "$($SitePrefix).local"
